...
int main() {
	...
	for (int time = 0; 1; time++) {
		...
		for (int i = 0; i < n; i++){
			if (process[i].btl != 0)
				cmp = 0;
			if (process[i].at <= time && process[i].pri < sht && process[i].btl != 0) {
				sht = process[i].pri;
				pos = i;
			}
		}
	...
	printf("%5s%4s%4s%5s%4s%5s%4s\n", "Pno", "AT", "BT", "PRI", "CT", "TAT", "WT");
	for (int i = 0; i < n; i++) {
		printf("%5d%4d%4d%5d%4d%5d%4d\n", process[i].pid, process[i].at, process[i].bt, process[i].pri, process[i].ct, process[i].tat, process[i].wt);
		tat += process[i].tat;
		wt += process[i].wt;
	}
